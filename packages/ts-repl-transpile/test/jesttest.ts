...
test("no 'use strict'", (t) => {
  ...
  console.log("the output", output);
  expect(output.code.startsWith("\"use strict\"")).toEqual( false);
});
...
test.only("simple export rewrite", (t) => {
  const output = transformSync(`export const foo = 42;`);
  console.log("the output", output);
  expect(output).toEqual(
    {
...
test("evaluable spans", (t) => {
  ...
  console.log("the output", output);
  expect(!!output.spans).toEqual( true);
});
...
export const foo = (a: number, b: number): Promise<E.Either<unknown,never>> =>
  ...
  console.log("the output", output);
  expect(output.spans[output.spans.length - 1]).toEqual({
    // is this off off?
    // end: 85,
    // start: 83,
    end: 84,
